...
from step00_shared_utils import project_root

DEFAULT_FASTA = str(project_root / "data" / "uniprot_sprot.fasta.gz")

USAGE_EXAMPLES = """\
examples:
  python run_all.py                                   all steps, default 500 aa / 6000 protein limits (test run)
  python run_all.py --steps 08 11 12                   run only specific steps
  python run_all.py --steps 13 14 --max-length 0       FULL length run: no aa limit, still capped at
                                                        --max-proteins (default 6000)
  python run_all.py --steps 13 --fasta data/my_proteins.fasta.gz --max-proteins 2000
...
def main():
    ...
    print(f"  Figures: {Path('figures').resolve()}")
